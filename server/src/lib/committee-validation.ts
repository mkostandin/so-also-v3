/**
 * Committee name validation utilities
 * Ensures committee names follow the required ALL CAPS format and patterns
 */
...
export class CommitteeValidator {
  ...
  static validateCommitteeName(name: string): ValidationResult {
    ...
    for (const pattern of INVALID_PATTERNS) {
      if (pattern.test(trimmedName)) {
        return {
          isValid: false,
          error: this.getErrorForInvalidPattern(trimmedName, pattern),
   ...
   */
  private static getErrorForInvalidPattern(name: string, pattern: RegExp): string {
